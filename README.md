...
загрузки приложение показывает это изображение и результат классификации:

- зеленый квадрат `NORMAL` - плата в нормальном состоянии;
- красный квадрат `DEFECT` - плата с дефектом.

При запуске Streamlit автоматически проверяет checkpoint:

```text
outputs/single_image_model.pt
```

Если checkpoint уже существует, модель не обучается заново. В консоль PyCharm
печатаются:

- путь к checkpoint;
- размер входа модели;
- количество данных в train/validation/test;
